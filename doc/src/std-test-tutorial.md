...
|`function record() public`| Starts recording storage reads/writes performed by subsequent calls. Subsequent calls to `record()` clear previous data (Foundry-compatible).|
|`function accesses(address c) public returns (bytes32[] memory reads, bytes32[] memory writes)`| Returns storage slots read/written by `c` since the last `record()`. Foundry-compatible semantics: every write also counts as a read, so written slots will appear in both arrays.|
